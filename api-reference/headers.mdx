...
| `X-Toolken-Metadata-*` | No | Flat-header shorthand for metadata. The suffix after `X-Toolken-Metadata-` is lowercased and converted from kebab-case to snake_case as the key (e.g. `X-Toolken-Metadata-Agent` sets `agent`, `X-Toolken-Metadata-Customer-Id` sets `customer_id`). Equivalent to sending the same key in `X-Toolken-Metadata`. The gateway also accepts the legacy `X-Toolken-Meta-*` form (e.g. `X-Toolken-Meta-Agent`) for backward compatibility; new integrations should use `X-Toolken-Metadata-*`. |
