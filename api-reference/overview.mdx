...
The Toolken Gateway API is a transparent pass-through proxy that sits between your application and the LLM provider. You point your existing SDK at the Toolken gateway URL instead of the provider URL -- everything else stays the same. The gateway adds cost attribution, budget enforcement, and usage recording without altering the request or response.
...
| `GET /v1/models` | OpenAI Models list | Defaults to OpenAI; override with `X-Provider` |
...
1. Explicit `X-Provider` header (e.g. `X-Provider: gemini`) -- always wins
...
4. Error -- cannot determine provider; attach `X-Provider` or include a recognizable model name
...
When the gateway itself cannot forward the request (budget exceeded, invalid key, timeout, etc.) it returns a JSON error body in the standard OpenAI-compatible shape. See [Errors](/api-reference/errors) for the full list of codes.
