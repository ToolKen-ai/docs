...
<Step title="Set X-Toolken-Meta-Agent to your agent's name">

Add `X-Toolken-Meta-Agent` to every request the agent makes. Use the agent's name as the value. The gateway records that value against every token used on the call.
...
summarizer = OpenAI(
    ...
    default_headers={
        "X-Toolken-Key": "tk_live_...",
        "X-Toolken-Meta-Agent": "summarizer",
    },
)
...
curl https://gateway.toolken.ai/v1/chat/completions \
  -H "Authorization: Bearer $OPENAI_API_KEY" \
  -H "X-Toolken-Key: tk_live_..." \
  -H "X-Toolken-Meta-Agent: summarizer" \
  -H "Content-Type: application/json" \
...
Add `X-Toolken-Meta-Customer-Id` alongside `X-Toolken-Meta-Agent` on the same request. The dashboard can then show cost per agent per customer, or cost per customer per agent, depending on how you group.
