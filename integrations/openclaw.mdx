...
Select the `toolken` provider and the `gpt-4o-mini` model (or whichever model you listed) when running your agent. OpenClaw routes the request through the gateway, attaching `X-Toolken-Key` and `X-Toolken-Meta-Agent` automatically.
...
`X-Toolken-Meta-Agent` is set at the provider level, not per request. To break spend down by agent, define one Toolken provider entry per agent and give each a distinct `X-Toolken-Meta-Agent` value:
  ...
  models: {
    providers: {
      ...
      toolken-summarizer: {
        ...
        headers: {
          "X-Toolken-Key": "${TOOLKEN_KEY}",
          "X-Toolken-Meta-Agent": "summarizer-agent"
        },
