...
Codex selects the `toolken` provider from `config.toml`, attaches `X-Toolken-Key` and `X-Toolken-Meta-Agent`, and routes the request through the gateway.
