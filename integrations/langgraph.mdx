...
`X-Toolken-Meta-Agent` labels this model's traffic in the dashboard. Use any string that identifies the graph or workflow.
...
<Tip>
To attribute spend per agent or per node, create one `ChatOpenAI` instance per logical role and give each a distinct `X-Toolken-Meta-Agent` value. For example, a `"planner-agent"` model and a `"summarizer-agent"` model each show up as separate line items in the dashboard, so you can see exactly which part of your graph is driving cost.
</Tip>
