...
client = anthropic.Anthropic(
    ...
    default_headers={
        "X-Toolken-Key": "tk_live_...",
        "X-Toolken-Meta-Agent": "support-agent",
...
Any request to `/v1/messages` routes to Anthropic automatically. A model name starting with `claude-` also triggers auto-detection. You do not need an `X-Provider` header.
