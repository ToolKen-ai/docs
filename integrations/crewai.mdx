...
`X-Toolken-Meta-Agent` is the metadata key that groups this agent's spend in the dashboard. Use any string that is meaningful to you.
...
<Tip>
Give each agent its own `LLM` instance with a distinct `X-Toolken-Meta-Agent` value to split spend per agent in the dashboard. For example, define one `LLM` for your researcher with `"X-Toolken-Meta-Agent": "research-agent"` and a separate one for your writer with `"X-Toolken-Meta-Agent": "writer-agent"`. The dashboard breaks cost and tokens down by agent so you can see exactly which agent is driving spend.
</Tip>
