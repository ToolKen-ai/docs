...
A managed LLM gateway on the edge. Point your client at one base URL and every call is attributed by feature, customer, and more, with budgets and rate limits enforced automatically. You bring your own provider keys.
</Accordion>
...
Budgets and rate limits fail open, so a brief disruption lets requests through rather than blocking them. Authentication fails closed, so requests are never forwarded without a valid key.
</Accordion>
