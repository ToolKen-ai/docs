...
description: "The gateway picks the right provider automatically. Use X-Provider to be explicit or to reach ambiguous models."
icon: "sliders"
---
...
<Step title="X-Provider header (if present)">
If you set `X-Provider`, that value wins immediately. An unknown value returns a `400 invalid_provider` error.
</Step>
...
<Step title="Error">
If none of the above resolves to a provider, the gateway returns `400 invalid_provider` and tells you to set `X-Provider`.
</Step>

</Steps>

## When to use X-Provider

Auto-detection covers most cases. Set `X-Provider` when:
...
The gateway validates that the provider supports the path you are calling. Anthropic only handles `/v1/messages`; every other provider handles the OpenAI-compatible paths (`/v1/chat/completions`, `/v1/embeddings`, etc.). Sending a `claude-` model to `/v1/chat/completions` without `X-Provider` is fine: the gateway detects Anthropic and returns `400 incompatible_path` because Anthropic requires `/v1/messages`. The error message tells you exactly what to fix.
...
| `together` | Together AI | Requires `X-Provider` |
| `minimax` | MiniMax | Auto-detected from `minimax-`, `abab-` prefixes |
| `xai` | xAI | Auto-detected from `grok-` prefix |
| `openrouter` | OpenRouter | Always requires `X-Provider` |
| `cerebras` | Cerebras | Requires `X-Provider` |
| `fireworks` | Fireworks | Requires `X-Provider` |
