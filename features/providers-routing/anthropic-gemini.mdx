...
Any request to `/v1/messages` routes to Anthropic automatically. A model name starting with `claude-` also triggers auto-detection to Anthropic. You do not need `X-Provider` for Anthropic unless you are sending to a path other than `/v1/messages`.
...
Model names starting with `gemini-` are auto-detected. No `X-Provider` header needed.
...
<CardGroup cols={2}>
  ...
  <Card title="Choose a Provider" icon="sliders" href="/features/providers-routing/choose-provider">Detection order and when to use `X-Provider`.</Card>
</CardGroup>
